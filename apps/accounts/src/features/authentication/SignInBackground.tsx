...
type BackgroundContainerCssProps = {
	height: number;
	width: number;
};

const BackgroundContainer = styled.div<BackgroundContainerCssProps>`
	${({ height, width }) => css`
		height: ${height}px;
		left: 50vw;
		position: fixed;
...
const gradient = {
	...
	8: '#f3ffbd',
	9: '#38ef7d',
	10: '#11998e', // eslint-disable-line sort-keys
};

export const SignInBackground: FC = () => {
	...
	const svgWidth = 1600;
	const svgHeight = 800;
	const shouldUseHeight = size.width / size.height < svgWidth / svgHeight;
	const containerWidth = shouldUseHeight
		? (size.height * svgWidth) / svgHeight
		: size.width;
	const containerHeight = shouldUseHeight
		? size.height
		: (size.width * svgHeight) / svgWidth;

	return (
		/** Adapted from https://www.svgbackgrounds.com/ */
		<BackgroundContainer height={containerHeight} width={containerWidth}>
			<Svg width="100%" height="100%" viewBox="0 0 1600 800">
				<rect fill={gradient[0]} width="1600" height="800" />
				<g fillOpacity="1">
					<path
						fill={gradient[1]}

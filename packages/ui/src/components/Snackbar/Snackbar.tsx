...
export const Snackbar = styled.div<SnackbarProviderProps>`
	${({ position, margin, minWidth, theme }) => {
		const left =
			position && ['top', 'bottom'].includes(position) ? '50vw' : null;
		...
		return css`
			bottom: ${position?.includes('bottom')
				...
				: ''};
			display: flex;
			flex-direction: ${position?.includes('top')
				? 'column'
				: 'column-reverse'};
			left: ${left ?? position?.includes('left')
				? margin?.left + 'px'
				: ''};
			...
			transform: ${position && ['top', 'bottom'].includes(position)
				? 'translateX(-50%)'
				: ''};

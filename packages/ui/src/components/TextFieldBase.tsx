...
const baseStyles = ({
	...
	return css`
		...
		> fieldset {
			...
			background-image: ${filledStyle &&
			`linear-gradient(${theme.palette.bg2}, ${theme.palette.bg2})`};
...
export const TextFieldBase = forwardRef<HTMLDivElement, InputBaseProps>(
		...
		return (
			<StyledTextField ref={ref} className={className} {...props}>
				<fieldset disabled={props.disabled}>
					{props.startIcon && (
						<span className={`${className}-start-icon`}>
							{props.startIcon}
						</span>
					)}
					...
					{props.endIcon && (
						<span className={`${className}-end-icon`}>
							{props.endIcon}
						</span>
					)}
